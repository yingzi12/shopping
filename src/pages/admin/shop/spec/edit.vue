...
const queryData = reactive({
  ...
  addForm: {
    ...
    rule: 1,
    shopId: '',
    prodPropValues: [{ valueId:"",propValue: '',propId:"" }],
  },
  rules: {},
});
...
async function getDetail() {
  const response = await api.get(`/admin/spec/info?propId=${id.value}`);
...
async function onSubmit() {
  ...
  const response = await api.post("/admin/spec/update", JSON.stringify(addForm.value));
  const data = response.data;
  //console.log(data)
  if (data.code == 200) {
    Dialog.create({
